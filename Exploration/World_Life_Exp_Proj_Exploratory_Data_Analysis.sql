...
WITH RollingTotals AS (
  ...
  SELECT 
    ...
    rolling_total,
    CASE 
      WHEN LAG(rolling_total) OVER(PARTITION BY Country ORDER BY Year) IS NOT NULL THEN 
        ROUND((rolling_total - LAG(rolling_total) OVER(PARTITION BY Country 
                 ORDER BY Year))/ LAG(rolling_total) OVER(PARTITION BY Country ORDER BY Year) * 100, 2)
        ELSE NULL 
      END AS rolling_total_rate_of_change
FROM RollingTotals
;
-- Insight: Average adult mortality rates have generally declined over the past 15 years, decreasing by 13.3% from 2007 to 2022. Periodic spikes may be linked to conflicts or natural disasters. 
